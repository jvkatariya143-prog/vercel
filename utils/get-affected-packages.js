...
async function getAllPackagesWithE2ETests() {
  try {
    // Use turbo to find all packages with e2e test tasks
    const output = await turbo(['ls', '--output=json']);
    const data = JSON.parse(output.toString('utf8'));

    const packagesWithE2E = data.packages
      .filter(pkg => {
        // Check if package has e2e test tasks
        const tasks = pkg.tasks || [];
        return tasks.some(
          task => task.includes('test-e2e') || task.includes('vitest-e2e')
        );
      })
      .map(pkg => pkg.name);
...
async function turbo(args) {
  ...
  return new Promise((resolve, reject) => {
    const root = path.resolve(__dirname, '..');
    const turboPath = path.join(root, 'node_modules', '.bin', 'turbo');
    const spawned = child_process.spawn(turboPath, args, {
      cwd: root,
      env: process.env,
    });
